...
then
  ...
  docker run \
      ...
      -e POSTGRES_MULTIPLE_DATABASES=auction \
      -p "${DB_PORT}":5432 \
      -d postgres-local \
      postgres -N 1000
fi
...
migrate -path ./auction-service/migrations -database "postgres://${DB_USER}:${DB_PASSWORD}@localhost:${DB_PORT}/auction?sslmode=disable" up
migrate -path ./bidding-service/migrations -database "postgres://${DB_USER}:${DB_PASSWORD}@localhost:${DB_PORT}/bidding?sslmode=disable" up
