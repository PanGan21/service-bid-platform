...
func (repo *auctionRepository) Create(ctx context.Context, auction entity.Auction) error {
	...
	const query = `
  		INSERT INTO auctions (Id, CreatorId, Info, Postcode, Title, Deadline, Status, WinningBidId) 
  		VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING Id;
	`

	c.QueryRow(ctx, query, auction.Id, auction.CreatorId, auction.Info, auction.Postcode, auction.Title, auction.Deadline, auction.Status, auction.WinningBidId).Scan(&auctionId)
	if err != nil {
...
func (repo *auctionRepository) UpdateOne(ctx context.Context, auction entity.Auction) error {
	...
	const query = `
  		UPDATE auctions SET CreatorId=$1, Info=$2, Postcode=$3, Title=$4, Deadline=$5, Status=$6, WinningBidId=$7 WHERE Id=$8 RETURNING Id;
	`

	c.QueryRow(ctx, query, auction.CreatorId, auction.Info, auction.Postcode, auction.Title, auction.Deadline, auction.Status, auction.WinningBidId, auction.Id).Scan(&auctionId)
	if err != nil {
		return fmt.Errorf("AuctionRepo - Create - c.Exec: %w", err)
	}
