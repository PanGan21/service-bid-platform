...
func (repo *auctionRepository) UpdateOne(ctx context.Context, auction entity.Auction) error {
	...
	const query = `
  		UPDATE auctions SET CreatorId=$1, Info=$2, Postcode=$3, Title=$4, Deadline=$5, Status=$6, WinningBidId=$7, RejectionReason=$8, WinnerId=$9, WinningAmount=$10 WHERE Id=$11 RETURNING Id;
	`

	c.QueryRow(ctx, query, auction.CreatorId, auction.Info, auction.Postcode, auction.Title, auction.Deadline, auction.Status, auction.WinningBidId, auction.RejectionReason, auction.WinnerId, auction.WinningAmount, auction.Id).Scan(&auctionId)
	if err != nil {
		return fmt.Errorf("AuctionRepo - Create - c.Exec: %w", err)
	}
...
func (repo *auctionRepository) FindOneById(ctx context.Context, id int) (entity.Auction, error) {
	...
	err = c.QueryRow(ctx, query, id).Scan(&auction.Id, &auction.Title, &auction.Postcode, &auction.Info, &auction.CreatorId, &auction.Deadline, &auction.Status, &auction.WinningBidId, &auction.RejectionReason, &auction.WinnerId, &auction.WinningAmount)
	if err != nil {
